        ...
        if (!isNull _display) then
        {
            ...
            if(missionNamespace getVariable ["BIS_AK_vanguardSpectator", false]) then
            {
                if(missionNamespace getVariable ["BIS_device_dropped", false]) then
                {
                    ...
                    private _mins = (floor (((missionNamespace getVariable ["BIS_endMatchTimer", 0]) - _timeElapsed)/60)) max 0;
                    private _secs = (((missionNamespace getVariable ["BIS_endMatchTimer", 0]) - _timeElapsed)-(_mins*60)) max 0;
